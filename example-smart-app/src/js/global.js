	...
	$.ajax({
		...
		success: function(data) { 
			...
			var temp2 = JSON.parse(tmp);
					...
					screen += "<div style=\'height: 50px; margin-top: 0.5em;\'><input type=\'button\' class=\'btn-submit\' id=\'" + item.modifierExtension[0].valueString + "\' name=\'" + item.text + "\' value=\'" + item.text + "\' onclick= \'nextQuestion( \"" +linkId+ "\",\"" +valueString+ "\",\"" +system+ "\",\"" +code+ "\", \"" +display+ "\",\"" +text+ "\",\"" +tempOID+ "\");  \' />" + "</div>";
